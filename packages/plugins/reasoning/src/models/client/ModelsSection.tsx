...
interface ProviderEditorRenderProps
  extends Pick<ProviderEditorProps, "namespace" | "api" | "t" | "readOnly" | "onClose"> {
  target: EditorTarget;
}
...
function targetOf(row: ProviderRow): EditorTarget {
  ...
  const credentialRef =
    row.apiKeyEnv === managedRef && row.credential?.configured === true && row.credential.writable
      ? managedRef
      : undefined;
  return {
...
function Loaded({ injected }: { injected: ModelsSectionInjected }): ReactNode {
  ...
  const addNamespace =
    addTarget === undefined ? undefined : state.namespaces.get(addTarget.settingsNs);
          ...
          <div className={styles.addCard}>
            <div className={styles.field}>
              ...
              <select
                ...
                onChange={(event) => {
                  const row = addable.find(
                    (candidate) => candidate.entry.provider === event.target.value,
                  );
