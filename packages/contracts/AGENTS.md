...
- **桥接不属于本包**：dsh 插件调用 Tauri 壳能力的桥接命令契约，由 `packages/plugins/bridge` 自持，
  Rust 侧在 `apps/shell/src-tauri/capabilities/bridge.json` 声明，不并入本包；`get_autostart` / `set_autostart` / `get_desktop_settings` / `set_desktop_settings` / 快捷键 / `check_update` / `install_update` 等命令当前主要经桥接使用，类型由 bridge 侧定义
