...
`Low / Medium / High / Very High / Extreme` 五档思考强度勾选。
...
- 在 pi-ai `ModelListEditor` 的模型行中加入思考强度勾选，按模型生成
  ...
  （`xhigh` / `max` 保留 pi-ai canonical id，只把显示名写成 Very High / Extreme）
...
- 输入栏推理等级由上游 `dsh-client-ui-model-selection` 提供；本插件只保证
  ...
  `ctx.llm.resolveModelInfo`，把 `xhigh` / `max` 显示为 Very High / Extreme
