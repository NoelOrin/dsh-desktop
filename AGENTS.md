...
5. **桌面壳设置**：经 `packages/plugins/bridge` 插件在 dsh WebUI 设置面板提供“桌面”页（状态 / 配置 / 工具 / 开机自启）与“外观”页（主题偏好 / 主题库 / 背景图 / 玻璃透明度 / 自定义主题 / 排版），通过 `window.__DSH_DESKTOP__` 桥接调用壳能力；原独立控制中心（SolidJS）已迁移至此并移除
...
7. **主题与无边框窗口**：壳侧轮询 `settings.yaml` 的 `ui-theme` 分节，通过 `get_ui_theme` / `dsh-ui-theme` 让启动页和窗口背景跟随；main 窗口无系统边框，启动页与 dsh web 注入自绘标题栏（拖动、双击最大化、窗口控制按钮）
