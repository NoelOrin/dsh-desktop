 ...
 * YAML-only; unchecking every intensity writes `{ off: null }` to close thinking.
 */
...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  ...
  const toggleEffort = (index: number, model: ModelDraft, id: EffortId): void => {
    ...
    if (Object.hasOwn(dict, id) && typeof dict[id] === "string") {
      delete dict[id];
    } else {
      dict[id] = id;
      delete dict[OFF_ID];
    }
    const stillOffers = EFFORT_CHOICES.some((choice) => typeof dict[choice.id] === "string");
    patch(index, { reasoningEfforts: stillOffers ? dict : { [OFF_ID]: null } });
  };
  ...
  return (
    <section className={styles.modelCatalog} aria-label={t("models")}>
      ...
      {models.map((model, index) => (
        <div
          key={typeof model.id === "string" ? model.id : String(index)}
          ...
          <fieldset className={styles.effortGroup}>
            ...
            <div className={styles.effortOptions}>
              {EFFORT_CHOICES.map((choice) => {
                const checked = typeof effortsOf(model)[choice.id] === "string";
                return (
                  <label className={styles.effortOption} key={choice.id}>
                    <input
                      ...
                      onChange={() => {
                        toggleEffort(index, model, choice.id);
                      }}
                    />
